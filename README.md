...
| 盘后正式候选池 / 次日盘中 100 池 | [reports/watchlists](reports/watchlists/) |
| 盘中 100 池刷新结果 | [reports/intraday_screening](reports/intraday_screening/) |
...
watchlist、盘中 100 池和 `track_stock.xlsx` 中的“建议总仓位%”就是按这个口径生成，用来限制最大计划仓位，不代表必须一次买满。
...
- 盘后正式选股优先用主 `watchlist`；盘中只看 `intraday_pool_screening_YYYY-MM-DD.csv` 和手动跟踪股，不从全市场临时重挑。
- P8 高分只提示三日短线弹性，风险很大，不单独构成买入理由；优先放进“Phase8 Top5短线观察”表等待主线和结构确认。
