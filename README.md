...
这类股票前面先走出过一段比较明确的主升，随后经过短暂过渡，进入平台整理，再放量突破平台上沿。平台本身必须更稳定：后半段平均成交量不超过前半段的 0.8，后半段振幅相对前半段继续收敛，后半段最低价不低于前半段最低价，且平台内不能出现“实体跌幅不低于 4%、成交量不低于平台均量 1.5 倍”的放量大阴线。突破日还需要满足收盘位置、上影线和实体质量要求，当前离平台高点不能太远。

它更像是在找“第一段主升之后，平台健康缩量蓄势，再次尝试打开空间”的票。
...
这类股票前面经历了由一个放量阳开启的主升，之后快速下降并回踩到倍量阳收盘支撑线附近，若支撑不破或跌破后快速重新站回，就进入反抽观察。回踩阶段要求整体缩量、后半段继续缩量，并排除放量大阴的出货嫌疑：单根回踩阴线实体跌幅达到 `4%` 且成交量达到回踩期均量 `1.5` 倍以上时，不再进入模式6。
...
注意：下表是 2026-04-24 当时规则下的历史快照。近期已经收紧了模式2/3的突破后时效和缩量要求、模式4的平台稳定性、模式5的均线斜率和回踩缩量条件，并为模式6加入了回踩阶段放量大阴排除，因此新版规则的统计表现需要重新运行 `backtest-patterns` 后再更新。
