...
1. 盘中数据更新及日中选股：盘中更新 `data/intraday` 临时日 K，只扫描盘后生成的 `intraday_pool` 和手动跟踪股，生成 `intraday_pool_screening` 日中参考。
...
2. 优先参考 [intraday_pool_screening_YYYY-MM-DD.csv](reports/intraday_screening/intraday_pool_screening_YYYY-MM-DD.csv)，不要从全市场模型预测文件里临时重挑；
