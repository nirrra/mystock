...
日中主表优先从 `intraday_top20_YYYY-MM-DD.csv` 里选。这个文件已经做过基础过滤，但过滤口径按来源分层：
...
这个分数只用于生成和排序 `intraday_top20_YYYY-MM-DD.csv`，不是新的模型预测目标。

正确用法：

- 先按主线归类。
- 同一主线内，优先选择 `intraday_focus_score` 更高、P1/P2 更接近 `80`、涨幅不过热的股票。
- 如果两只股票 Phase4 接近，优先选 P1/P2 更接近 `80` 的一只。
- 主线不匹配时，即使 Phase4 或 focus 分很高，也不要压过主线内质量合格的股票。
...
| 股票代码 | 股票名称 | 主线 | P1/P2 | P4 | Focus | 盘中涨幅 | 建议总仓位 | 技术/前日形态 | 日中判断 |
| -------- | -------- | ---- | ----- | --: | ----: | -------: | -----------: | ------------- | -------- |
| 000000 | 示例股票 | 算力硬件 | 80 / 78 | 86 | 105.5 | 2.8% | 28.7% | 前日模式2 + 金叉 | 属于当前主线，P1/P2 接近 80，focus 排序靠前，涨幅未过热，尾盘重点看能否站稳关键位。 |

手动跟踪股票总结：

| 股票代码 | 股票名称 | 是否日中关注 | P1/P2 | P4 | Focus | 盘中涨幅 | 建议总仓位 | 判断 |
| -------- | -------- | ------------ | ----- | --: | ----: | -------: | -----------: | ---- |
| 000000 | 示例股票 | 观察 | 55 / 48 | 62 | 70.3 | 1.5% | 21.2% | 在跟踪列表内但不属于当前第一主线，P2 低于自动入表底线，尾盘只观察是否放量转强。 |

不选或降级：

- 000000示例股票：P4 很高但 P1/P2 一般，不属于当前主线，且盘中涨幅接近 8%，只观察不追。
```
